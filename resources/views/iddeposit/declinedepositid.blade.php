...
<header style="overflow: hidden;">
    ...
    <a href="{{ url('/admin/depositid') }}" class="btn btn-info float-left">Deposit List</a>

</header>
<section class="gateway_view">
  <form action="{{ url('/admin/createapi/declinedepositid') }}" method="POST" enctype="multipart/form-data">
<br>
...
<div class="alert alert-info">
  ...
  <div class="form-row">
    ...
    <div class="form-group col-md-6">
    	...
    	<h6>Deposit From: {{$deposit->deposittype}}</h6>
    	<h6>Deposit ID: {{$deposit->depositid}}</h6>
    	<h6>Wallet Balance: {{$deposit->wallet}}</h6>
    	<h6>ID Balance: {{$deposit->balance}}</h6>
    	<h6>Current Status: {{$deposit->status}}</h6>
    	</div>
    </div>
  @if ($deposit->status!='declined')
