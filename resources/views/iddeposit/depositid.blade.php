...
<header style="overflow: hidden;">
	<div class="row">
		...
		<div class="col-4">
	<a href="{{ url('/admin/depositidcreate') }}" class="btn btn-info float-right">Create Id Deposit</a>
  <a href="{{ url('/admin/deposit') }}" class="btn btn-warning float-right">Wallet deposit</a>
    ...
    .done(function(data) {
     var d = JSON.parse(data);
      ...
      for (var i = 0; i < d[0].length; i++) {
        ...
        <h5>Deposit From: `+row['deposittype']+`</h5>
        </div>
        <div class="col-2 center" style='padding-top: 15px;'>
...
<button onclick='deletes(`+row['id']+`)' class='btn btn-danger'>Delete</button>
  ...
  function edit(id)
  {
   if (confirm("Are you sure you want to approve the deposit?")) {
    $.ajax({
      url: '{{ url('/admin/approvedepositid') }}',
      type: 'POST',
  ...
  function decline(id)
  {
   window.location = '{{ url('/admin/declinedepositid/') }}/'+id;
  }
  function details(id)
  {
   window.location = '{{ url('/admin/depositid/') }}/'+id;
  }
