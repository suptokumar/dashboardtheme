...
class depositcontroller extends Controller
{
    ...
    {
        if (soft::power(Auth::id(),"deposit",0)) {
            return view("iddeposit.depositid");
        }else{
            return redirect("permissionerror");
        }
    }
    ...
    public function deletedepositid(Request $request)
    {
        if (soft::power(Auth::id(),"deposit",0)) {
            $deposit = iddeposit::find($request->get("id"));
            if ($deposit->delete()) {
                soft::send($deposit->username,$deposit->phone, "Your ID Deposit request of ".$deposit->amount." ".$deposit->currency." was deleted", Auth::user()->name,0);
                return __("Deleted");
            }else{
                return __("Failed");
            }
    ...
    public function declinedepositidpost(Request $request)
    {
        if (soft::power(Auth::id(),"deposit",0)) {
            $deposit = iddeposit::find($request->get("id"));
            $deposit->status = "declined";
            if ($deposit->save()) {
                soft::send($deposit->username,$deposit->phone, "Your ID Deposit request of ".$deposit->amount." ".$deposit->currency." was declined. cause ".$request->get("declinecause"), Auth::user()->name,0);
               return back();
            }else{
                return back();
        }
    ...
    public function approvedepositid(Request $request)
    {
        if (soft::power(Auth::id(),"deposit",0)) {
            $deposit = iddeposit::find($request->get("id"));
            $deposit->status = "approved";
            if ($deposit->save()) {
                soft::send($deposit->username,$deposit->phone, "Your ID Deposit request of ".$deposit->amount." ".$deposit->currency." was approved", Auth::user()->name,0);
            return __("Approved");
            }else{
                return __("Failed");
        }
    ...
    public function depositidcreate()
    {
        if (soft::power(Auth::id(),"deposit",0)) {
            $user= User::where("role","=","User")->get();
            $gateway = paymentdetail::get();
            return view("iddeposit.depositidcreate",['users'=>$user,'gateway'=>$gateway]);
        }else{
            return redirect("permissionerror");
        }
    }
    ...
    public function declinedepositid($id)
    {
        if (soft::power(Auth::id(),"deposit",0)) {
            $deposit = iddeposit::where("id","=",$id)->first();
            $deposit->wallet = intval(soft::wallet($deposit->phone));
            $deposit->balance = intval(soft::idbalance($deposit->id));
            return view("iddeposit.declinedepositid",['deposit'=>$deposit]);
        }else{
            return redirect("permissionerror");
        }
    }
    public function depositdetails($id)
    {
        if (soft::power(Auth::id(),"deposit",0)) {
            $deposit = deposit::where("id","=",$id)->first();
            $idm = idmanager::where("phone","=",$deposit->phone)->get();
            foreach($idm as $key => $ids){
                $idm[$key]->balance = soft::idbalance($ids->id);
            }
            $user = user::where("phone","=",$deposit->phone)->first();
            return view("admin.depositview",['deposit'=>$deposit,'user'=>$user,'idm'=>$idm]);
        }else{
            return redirect("permissionerror");
        }
    }

    public function depositiddetails($id)
    {
        if (soft::power(Auth::id(),"deposit",0)) {
            $deposit = iddeposit::where("id","=",$id)->first();
            $user = user::where("phone","=",$deposit->phone)->first();
            ...
            return view("iddeposit.iddepositview",['deposit'=>$deposit,'user'=>$user,'idm'=>$idm]);
        }else{
            return redirect("permissionerror");
        }
    }
    ...
    public function adddepositid(Request $request)
    {
        if (soft::power(Auth::id(),"deposit",0)) {
...
WHEN phone LIKE '__________$search%' THEN 11
 END,
    ...
    public function load_iddeposit(Request $request)
    {
        if (soft::power(Auth::id(),"deposit",0)) {
                $search = $request->get("search");
        $type = $request->get("type");
        if ($type=='') {
        $and = "iddeposits WHERE (`username` LIKE '%$search%' OR `depositid` LIKE '%$search%' OR `phone` LIKE '%$search%')";
        }else{
        $and = "iddeposits WHERE (`username` LIKE '%$search%' OR `depositid` LIKE '%$search%' OR `phone` LIKE '%$search%') AND status='$type'";
        }
...
WHEN phone LIKE '__________$search%' THEN 11
 END,
        ...
        foreach ($result as $c => $value)
        {
            $result[$c]->created_at = date("Y/m/d h:i a", strtotime($value->created_at));
            $result[$c]->updated_at = date("Y/m/d h:i a", strtotime($value->updated_at));
            $result[$c]->wallet = soft::wallet($value->phone);
            // $result[$c]->idm = idmanager::find($value->idm)->username;
