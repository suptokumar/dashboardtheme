...
WHEN email LIKE '__________$search%' THEN 11
 END,
    ...
    public function addexchangelistintodatabase(Request $request)
    {
        if ($this->power(Auth::id(),"exchange",0)) {
         ...
         if ($request->hasFile('file'))
            ...
            $exchange->type = exchangeconfigure::find($request->get("type"))->first()->text;
            $exchange->type_id = $request->get("type");
            $exchange->type_image = exchangeconfigure::find($request->get("type"))->first()->logo;
            if ($exchange->save()) {
    ...
    public function load_exchange(Request $request)
    {
        ...
        $and = "exchanges WHERE (`name` LIKE '%$search%') AND type_id='$type'";
        }
...
WHEN name LIKE '__________$search%' THEN 11
 ...
 END,
        ...
        foreach ($result as $key => $value) {
            if ($configure = exchangeconfigure::find($value->type_id)) {
