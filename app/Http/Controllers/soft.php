...
WHEN phone LIKE '__________$search%' THEN 11
 END,
    ...
    public static function wallet($id)
    {
        ...
        $iddeposit = iddeposit::where([["phone","=",$id],["status","=",'approved'],["deposittype","=",'wallet']])->get();
        foreach ($iddeposit as $key => $value) {
            if ($value->currency == 'INR') {
                $account-= $value->amount;
            }
        }

        $idwithdrawal = idwithdrawal::where([["phone","=",$id],["status","=",'approved'],["deposittype","=",'wallet']])->get();
        foreach ($idwithdrawal as $key => $value) {
            if ($value->currency == 'INR') {
                $account+= $value->amount;
            }
        }
    ...
    public static function idbalance($id)
    {
        $account = 0;
        $iddeposit = iddeposit::where([["idm","=",$id],["status","=","approved"]])->get();
        foreach ($iddeposit as $key => $value) {
            if ($value->currency == 'INR') {
                $account+= $value->amount;
            }
        }
        $idwithdrawal = idwithdrawal::where([["idm","=",$id],["status","=",'approved']])->get();
        foreach ($idwithdrawal as $key => $value) {
            if ($value->currency == 'INR') {
                $account-= $value->amount;
            }
