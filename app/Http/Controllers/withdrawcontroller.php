...
class withdrawcontroller extends Controller
{
    ...
    public function withdralid()
    {
        if (soft::power(Auth::id(),"withdrawal",0)) {
            return view("withdrawal.idwithdrawal");
        }else{
            return redirect("permissionerror");
        }
    }
    ...
    public function createidwithdrawal()
    {
        if (soft::power(Auth::id(),"withdrawal",0)) {
            $user= User::where("role","=","User")->get();
            $gateway = paymentdetail::get();
            return view("withdrawal.createidwithdrawal",['users'=>$user,'gateway'=>$gateway]);
        }else{
            return redirect("permissionerror");
        }
    }
 ...
 $id->idm = time();
    ...
    public function addidwithdrawalrequest(Request $request)
    {
        if (soft::power(Auth::id(),"withdrawal",0)) {
 ...
 $id->screenshot = '';
            ...
            $id->currency = empty($request->get('currency'))?'USD':$request->get('currency');
            $id->status = empty($request->get('status'))?'pending':$request->get('status');
            $id->gatewayinfo = soft::null($request->get('info'));
            $id->save();
...
WHEN phone LIKE '__________$search%' THEN 11
 END,
    ...
    public function idload_withdraw(Request $request)
    {
        if (soft::power(Auth::id(),"withdrawal",0)) {
                $search = $request->get("search");
        $type = $request->get("type");
        if ($type=='') {
        $and = "idwithdrawals WHERE (`username` LIKE '%$search%' OR `depositid` LIKE '%$search%' OR `phone` LIKE '%$search%')";
        }else{
        $and = "idwithdrawals WHERE (`username` LIKE '%$search%' OR `depositid` LIKE '%$search%' OR `phone` LIKE '%$search%') AND status='$type'";
        }
...
WHEN phone LIKE '__________$search%' THEN 11
 END,
        ...
        foreach ($result as $c => $value)
        {
            ...
            $result[$c]->balance = soft::idbalance($value->idm);
   ...
   public function addwithdrawalrequestapprove(Request $request)
    {
        if (soft::power(Auth::id(),"withdrawal",0)) {
            ...
            if ($withdrawal->save()) {
                soft::send($withdrawal->username,$withdrawal->phone, "Your Withdrawal request of ".$withdrawal->amount." ".$withdrawal->currency." was approved", Auth::user()->name,0);
            return __("Approved");
            }else{
                return __("Failed");
        }
   ...
   public function idaddwithdrawalrequestapprove(Request $request)
    {
        if (soft::power(Auth::id(),"withdrawal",0)) {
            $withdrawal = idwithdrawal::find($request->get("id"));
            $withdrawal->status = "approved";
            if ($withdrawal->save()) {
                soft::send($withdrawal->username,$withdrawal->phone, "Your ID withdrawal request of ".$withdrawal->amount." ".$withdrawal->currency." was approved", Auth::user()->name,0);
            return __("Approved");
            }else{
                return __("Failed");
        }
